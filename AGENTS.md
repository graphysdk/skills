...
**Don't put real skill files under `plugin-skills/`.** It's plumbing for the `/plugin` channel: each entry symlinks to a canonical skill under `skills/`, with the symlink directory named to control how the plugin namespaces it (e.g. `plugin-skills/install → ../skills/graphy-sdk-install` makes the skill invokable as `/graphy:install` while staying `/graphy-sdk-install` via the Vercel CLI). When you add a new skill, update both: write the file under `skills/`, then add a symlink under `plugin-skills/` if you want it exposed via `/plugin` too.
...
- `packages/react-renderer/src/index.ts` — renderer public API
- `packages/viz-engine/src/types/graph-config.ts` — `GraphConfig` type
- `packages/react-renderer/src/graph-renderer.tsx` — `GraphRenderer` props
