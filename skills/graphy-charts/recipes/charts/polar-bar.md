...
| Count labels | `geom.bar({ dataLabels: { showDataLabels: true, format: 'absolute' } })` — polar bars default to `'percentage'` |
...
Both stacked variants have touching geoms, so a bare `scale.color.palette()` (`{ type: 'default' }`) resolves to the single-hue `brick` mono ramp for the whole chart; the dodged rose gets the 8-colour set. Ask for multicolour with `scale.color.palette({ palette: { type: 'graphy' } })`.
...
Data labels — `format` defaults to `'percentage'` on any polar bar (share of the layer total); pass `format: 'absolute'` for counts. `showStackTotals` warns `DATA_LABEL_SETTING_IGNORED` under polar:
